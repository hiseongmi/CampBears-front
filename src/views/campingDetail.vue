...
<template>
  <section class="detail-camping">
    ...
    <div class="map-box">
      ...
      <button @click="ck">{{ infoData.tel }}</button>
    </div>

    <!--    <div id="map" style="width: 55%; height: 350px; margin: 0 auto; z-index: 0"></div>-->

    <img :src="infoData.thumbNailUrl ? infoData.thumbNailUrl : 'assets/image/backgroundImg.webp'" alt="main">
    <div class="name">{{ infoData.campingName }}</div>
    <div class="des">{{ infoData.campingIntro }}</div>
    <div class="camp-type">
              <span>
              {{ infoData.campingManageMode }}
            </span>
    </div>
    <div>캠핑 타입 :{{ infoData.campingTypes }}</div>
    <div>주소 : {{ infoData.address }}</div>
    <div>전화번호 : {{ infoData.tel }}</div>
    <div>생성일 : {{ infoData.createdDate }}</div>
  </section>
</template>
